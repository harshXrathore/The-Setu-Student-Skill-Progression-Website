...
export function StudentRecordTabs({
  ...
  const handleAddItem = async (
    type: "notes" | "assignments" | "resources",
    payload: any,
  ) => {
    if (!studentId) return;
    try {
      ...
      <TabsContent value="notes" className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          {record.notes?.map((n: any, i: number) => (
            <div
              key={i}
              className="p-3 bg-secondary/20 border border-border rounded-lg text-sm"
            >
                ...
                {a.status === "pending_review" && (
                  <div className="mt-3 flex gap-2 border-t pt-3">
                    <Button
                      size="sm"
                      onClick={() =>
                        handleUpdateAssignmentStatus(a._id, "completed")
                      }
                      className="bg-green-600 hover:bg-green-700 text-white h-7 py-0 text-xs px-3"
                    >
                      Approve & Give XP
                    </Button>
                    <Button
                      ...
                      variant="outline"
                      onClick={() =>
                        handleUpdateAssignmentStatus(a._id, "pending")
                      }
                      className="h-7 py-0 text-xs px-3 border-orange-200 text-orange-700 hover:bg-orange-50 bg-white shadow-sm"
                    >
                      Request Revision
                    </Button>
                  </div>
