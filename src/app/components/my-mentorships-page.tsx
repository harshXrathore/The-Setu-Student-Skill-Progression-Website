    ...
    <div className="space-y-6 animate-in fade-in zoom-in duration-300">
      <div>
        ...
        <p className="text-muted-foreground">
          View assignments and resources shared by your mentors.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-8">
          ...
          {sessionsList.filter((s) =>
            ["Pending", "Confirmed", "Pending Reschedule"].includes(s.status),
          ).length > 0 && (
            <div className="space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {sessionsList
                  .filter((s) =>
                    ["Pending", "Confirmed", "Pending Reschedule"].includes(
                      s.status,
                    ),
                  )
                  .map((session) => (
                    <div
                      key={session._id}
                      className="p-5 rounded-2xl border border-border bg-card shadow-sm hover:shadow-md transition-all flex flex-col justify-between gap-4"
                    >
                      <div>
                        <div className="flex items-start justify-between mb-2">
                          <h4 className="font-bold text-lg text-foreground truncate pl-1">
                            {session.topic}
                          </h4>
                          <Badge
                            variant={
                        ...
                        {session.status === "Confirmed" ? (
                          session.meetingUrl ? (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="w-full flex items-center justify-center py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-colors shadow-sm gap-2"
                            >
                          ...
                          <div className="flex w-full gap-2">
                            <button
                              onClick={async () => {
                                try {
                                  await apiRequest(
                                    `/mentors/sessions/${session._id}/status`,
                                    {
                                      method: "PUT",
                                      body: JSON.stringify({
                                        status: "Confirmed",
                          ...
                          <div className="w-full py-2.5 bg-secondary/50 text-muted-foreground font-semibold rounded-xl text-center text-sm border border-border border-dashed">
          ...
          {records.length > 0 ? (
                ...
                <Card key={record._id} className="overflow-hidden">
                  <CardHeader className="bg-secondary/30 border-b border-border pb-6 flex flex-row items-center gap-4 space-y-0">
                    <Avatar className="h-12 w-12 border-2 border-background shadow-sm">
                      {record.mentor?.avatar && (
                        <AvatarImage src={record.mentor.avatar} />
                      )}
                      <AvatarFallback>
                        {(record.mentor?.name || "M").slice(0, 2).toUpperCase()}
                      </AvatarFallback>
                    </Avatar>
                    <div>
                      ...
                      {record.assignments?.length === 0 ? (
                        ...
                        <div className="space-y-3">
                          {record.assignments?.map((a: any) => (
                            <div
                              key={a._id}
                              className={`p-4 border rounded-xl transition-colors ${a.status === "completed" ? "bg-secondary/50 border-border/50" : "bg-background hover:border-primary/50 shadow-sm"}`}
                            >
                              <div className="flex items-start gap-3">
                                <button
                                  ...
                                  onClick={() =>
                                    handleCompleteAssignment(a._id)
                                  }
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="flex justify-between items-start">
                                    <h4
                                      className={`font-medium ${a.status === "completed" ? "text-muted-foreground line-through" : "text-foreground"}`}
                                    >
                                      {a.title}
                                    </h4>
                                    {a.dueDate && (
                                      <Badge
                                        variant="outline"
                                        className={`ml-2 whitespace-nowrap ${new Date(a.dueDate) < new Date() && a.status !== "completed" ? "text-red-500 border-red-200 dark:border-red-900 bg-red-50 dark:bg-red-950/30" : ""}`}
                                      >
                                  ...
                                  {a.attachmentUrl && (
                                    <a
                                      href={`${import.meta.env.VITE_API_URL || "https://the-setu-student-skill-progression.onrender.com"}${a.attachmentUrl}`}
                                      target="_blank"
                                  ...
                                  <div className="mt-3 flex gap-2">
                                    ...
                                    a.completedDate ? (
                                      ...
                                      <Badge
                                        variant="secondary"
                                        className="text-xs bg-yellow-100 text-yellow-800 dark:bg-yellow-900/40 dark:text-yellow-400"
                                      >
                                        Under Review
                                      </Badge>
                                    ) : (
                              ...
                              <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="mt-2 text-xs text-muted-foreground font-medium">
                                    Added{" "}
                                    {format(new Date(r.sharedDate), "MMM d")}
                                  </div>
                                </div>
                              </div>
                            </a>
            ...
            <div className="text-center py-12 bg-card rounded-xl border border-border border-dashed mt-4">
              ...
              <p className="text-muted-foreground mt-1 text-sm">
                You haven't been assigned any tasks or resources by a mentor
                yet.
              </p>
            </div>
          )}
